...
システムは二段構えのマスキング機構（CloudWatch Logsのデータ保護機能とBedrock Guardrails）を備え、個人情報や金融情報が推論経路に流れ込む前に除去します。Step Functionsがモデル選択を明示的にルーティングし、Haikuでトリアージ、Sonnetで深堀り、Opusで最重大インシデントの根本原因分析を行います。Haikuへの入力はログprefilterでトークン消費を約7割削減し、`severity_hint=P1` のsignalで判断のブレを抑えます。生成された調査結果は日本語でDynamoDBにインシデントレコードとして記録され、`[IRA][P1]` / `[IRA][P2]` / `[IRA][P3]` の件名プレフィックス付きでSNS経由でオンコール担当者にメール通知されます。

企業向けガバナンス層としてGuardDutyとAWS Configを最小構成で組み込んでいます。GuardDutyはアカウント全体の脅威検知、AWS Configは構成ドリフト検知を担当し、少人数検証から社内展開に広げる過程でのリスク管理に使えます。

本リポジトリにはTerraformによるIaC一式と、障害検知の動作検証用に適時障害を発生させるNode.jsサンプルアプリケーションが含まれています。ECR / ECS Fargate上で稼働し、HTTP 5xx、応答遅延、OOM、外部API呼び出し失敗、エラーログ急増の5種類の通常障害に加えて、Opusまでエスカレーションさせるための `POST /chaos/p1` エンドポイントも公開しています。
...
## ガバナンス層（GuardDuty / AWS Config）

社内向けに展開する場合、一次トリアージの自動化だけでは不十分です。少人数の検証から全社展開へ広げる途中で、構成の手動変更と脅威検知の2方向からリスクが顕在化します。本システムでは次の2サービスを最小構成で組み込んでいます。

- **GuardDuty**: detectorを有効化し、既存のVPC Flow LogsとCloudTrail管理イベントをsourceとして取り込みます。`finding_publishing_frequency` を `FIFTEEN_MINUTES` に設定し、severity 7.0以上のfindingsをEventBridge経由で既存のインシデント用SNSに流し込みます。
- **AWS Config**: `configuration_recorder` と `delivery_channel` を有効化し、managed rulesを4個登録します。`IAM_PASSWORD_POLICY`、`INCOMING_SSH_DISABLED`、`S3_BUCKET_PUBLIC_READ_PROHIBITED`、`EC2_SECURITY_GROUP_ATTACHED_TO_ENI` の4本で、Terraformで宣言した「あるべき状態」からのドリフトを検知します。

両方とも `enable_guardduty` / `enable_config` の変数で有効/無効を切り替えられるため、既にアカウントで別プロジェクトが有効化している場合は `false` に設定します。

`aws_config_delivery_channel` に `sns_topic_arn` を付けるとmanaged rule初回評価でNON_COMPLIANTリソース数だけメールが飛ぶため、本実装では付けていません。コンプライアンス状態はAWSコンソールのダッシュボードで確認します。
...
├── README.md                  本ファイル
├── .githooks/
│   └── pre-commit             全 workspace を format/lint/test/build で検証する厳密版フック
├── docs/
...
│   ├── outputs.tf
│   ├── providers.tf
│   ├── modules/               ネットワーク / chaos-app / agt-sidecar / 観測 / パイプライン / 保存 / 通知 / 予算 / guardduty / config_governance
│   └── envs/dev/
├── apps/
│   ├── chaos-app/             Node.js 障害サンプルアプリ（P1 誘発エンドポイント含む）
│   └── agt-sidecar/           Agent Governance Toolkit 出口プロキシ
├── lambda/                    Bedrock 呼び出し Lambda (Haiku / Sonnet / Opus、日本語prompt + prefilter)
│   └── src/shared/prefilter.ts  Haiku 入力のトークン圧縮
└── scripts/
    ├── install-hooks.sh       pre-commit フック有効化（クローン直後に1回実行）
    ├── deploy.sh              一括デプロイ
    ├── verify.sh              動作検証
    ├── trigger-chaos.sh       手動障害発生
    └── destroy.sh             一括削除（Lambda zip 自動再生成付き）
```
...
```bash
# 初回のみ: pre-commit フック有効化
./scripts/install-hooks.sh

# 一括デプロイ（Lambdaビルド、ECRへのpush、全Terraform apply）
./scripts/deploy.sh
...
# Opusまでエスカレーションする P1 誘発
ALB=$(cd terraform/envs/dev && terraform output -raw alb_dns_name)
curl -X POST "http://$ALB/chaos/p1"

# 動作確認後は全リソースを削除
./scripts/destroy.sh
```

初回の `scripts/deploy.sh` 実行後、`terraform.tfvars` に指定したメールアドレスへSNS購読確認メールが届きます。本文のリンクを踏んで購読を確定させないと後続のインシデント通知が受信できません。
...
このフックはcommit時に全workspace（`apps/chaos-app` / `apps/agt-sidecar` / `lambda`）で `format:check`、`lint`、`test`、`build` を完全実行します。遅さより完全性を優先する厳密版のため、ローカルでも実行に数十秒〜 1 分程度かかります。
...
- AWS Configは region あたり1つしかrecorderを持てません。既に別プロジェクトで有効化済みの場合は `enable_config = false` に設定してください。GuardDutyも同様に `enable_guardduty` で制御できます。
- `terraform destroy` は `aws_lambda_function.source_code_hash = filebase64sha256(...)` を評価するため、`lambda/dist/*.zip` がローカルに無いと止まります。`scripts/destroy.sh` は冒頭でzip存在を確認し、無ければ自動再生成します。
